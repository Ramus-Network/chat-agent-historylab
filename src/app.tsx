...
export default function Chat() {
  ...
  // Function to export conversation as markdown file
                ...
                if (result.status === 'success' || result.status === 'partial_success') {
                  if (result.documents && result.documents.length > 0) {                    
                    ...
                    result.documents.forEach((doc: any, index: number) => {
                      // Debug log to inspect document structure
                      // console.log("doc", doc);
  ...
  // Add this array of example queries before the return statement
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        {randomizedQueries.map((item, index) => (
                          <button
                            key={index}
                            onClick={() => {
                              handleAgentInputChange({ target: { value: item.query } } as React.ChangeEvent<HTMLTextAreaElement>);
                              setTimeout(() => {
